...
if (sideSearch) sideSearch.addEventListener("click", () => window.kSearch && window.kSearch.open());
...
// Expose a hook for the Cmd+K palette to open the decision modal for any
// item key without needing a row reference. Called after the route lands
// on the target BU.
window.openDecisionModalFromSearch = async function (key, buSlug) {
  try {
...
async function refreshAllHandler(e) {
  ...
  try {
    ...
    _busCache = null; window.dispatchEvent(new Event("bus:updated"));
...
function renderItemRow(it) {
  ...
  const rowClass = [
    overridden ? "has-decision" : "",
    isDelta ? "is-delta" : ""
  ].filter(Boolean).join(" ");
  // The verdict cell shows: "AI: X → You: Y" when the human disagreed
  // with the AI; just the current verdict otherwise.
...
function bindBuEvents(bu) {
  ...
  // decision modal.
  // SHIFT-click on a verdict button = quick-decide (skip modal, log with
  // empty reason). Saves a lot of clicks when you're agreeing with the AI.
  document.querySelectorAll("table.items tbody tr").forEach(tr => {
    tr.addEventListener("click", (e) => {
      ...
      // Bare row click. Ignore if user clicked the external Jira link or
...
// Quick-decide: save a verdict without opening the modal. Reason is empty.
// Used by shift-click on row buttons.
...
// Bulk decide: prompt for one reason, apply the same verdict to every
// selected row. Reuses the single-item endpoint serially.
